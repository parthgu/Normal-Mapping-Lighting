...
void main(void)  {
    ...
    if (textureColor.a > 0.9) {
        ...
        // vec3 specular = pow(max(0.0, dot(normalize(uCameraPos - vFragPos), lightRefle
        vec3 specular = max(0.0, dot(normalize(uCameraPos - vFragPos), lightReflect)) * 0.5 * lightColor;
